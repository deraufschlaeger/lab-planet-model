...
def give_radius_itercount_from_layernum(layer_multiplier, num_hydrogen_layers=180, he_ab=0.25, num_mix_layers = 10, temp = 160, core_mass_ratio=0.2, num_core_layers = 10, mass_total = 1.898e27, rho_0 = 1300, quiet=True):
    ...
    r_profile, p_profile, rho_profile, temp_profile, iter_counter, radius_array = multi_iteration(rho_0, mass_total, N, temp, eos_profile, quiet=quiet, max_iter = 40, mass_distro_profile=mass_dictionary)
    if quiet==False:
        print(core_mass_ratio)
        print(r_profile[-1][-1])

    return r_profile[-1][-1], iter_counter, radius_array
...
radius, itercounter, radius_for_each_iteration = give_radius_itercount_from_layernum(multiplier, num_core_layers=num_core_layers, num_mix_layers=num_mix_layers, num_hydrogen_layers=num_hydrogen_layers)

fig, ax = plt.subplots()

iter_array = list(range(1,itercounter+1))
from matplotlib.ticker import MaxNLocator
ax.plot(iter_array, np.array(radius_for_each_iteration)/1e3)
